...
function displayM(meals) {
    const mealContainer = document.getElementById('meals');
    mealContainer.innerHTML = '';

    if (meals && meals.meals) {
        meals.meals.forEach(area => {
            const truncatedDescription = area.strDescription.split(' ').slice(0, 20).join(' ');
            mealContainer.innerHTML += `
                <div class="col position-relative overflow-hidden text-center" role="button" onclick="displayMeals(event)">
               <i class="fa-solid fa-drumstick-bite fa-4x"></i>
                    <h2>${area.strIngredient}</h2>
                    <p>${truncatedDescription}</p>
                </div>
            `;
        });
    } else {
        console.log('No meal areas found.');
    }
}
fetchMeals();
...
function displayMeals(event) {
    const areaName = event.currentTarget.querySelector('h2').textContent;
    const apiUrl = `https://www.themealdb.com/api/json/v1/1/filter.php?i=${areaName}`;
